from .twin_sac_q import TwinSACQ, SerialTwinSACQ
import copy
import torch
import numpy as np
...
class MTSAC(TwinSACQ):
    ...
    def __init__(self, task_nums,
        ...
        if self.automatic_entropy_tuning:
            # log_alpha starts from zero, and needs tobe learned during upcoming training
    ...
    def update(self, batch):
        ...
        # means whether to use embeddings of tasks as inputs
        if self.pf_flag:
            embedding_inputs = batch["embedding_inputs"]

        # the index for different tasks in MT10 or MT50
        if self.idx_flag:
            task_idx    = batch['task_idxs']
            # print(task_idx)

        # move to cuda(if cuda is enabled)
        ...
        # print("obs:", obs.shape)  [batch_size, 1, 6]
        # print("actions:", actions.shape)  [batch_size, 1, 4]
        # print("terminals: ", terminals.shape)  [batch_size, 1, 1]
        # print("next_obs: ", next_obs.shape)   [batch_size, 1, 6]
        # print("rewards: ", rewards.shape) [batch_size, 1, 1]
        ...
        # use the original obs sampled from replay buffer, feed through the policy network
        # to get a new action, and mean log_std log_prob
        if self.idx_flag:
        ...
        # get current q-value prediction based on current observations and actions
        if self.idx_flag:
            # print("idx_flags: ", task_idx)
        ...
        # TODO: what is automatic entropy tuning
        if self.automatic_entropy_tuning:
            """
            Alpha Loss
            """
        ...
        # use next obs to feed through the policy network, to get a newer action prediction
        with torch.no_grad():
            if self.idx_flag:
        ...
        # soft update: target_param = target_param * (1-tau) + param* tau
        # hard update: target_param = param
    ...
    def update_per_epoch(self):
        # opt_times = 200: sample 200 times from replay buffer, and update on that batch
        for _ in range(self.opt_times):
            
            # batch size = 1280
            # sample keys: the tags to fetch from replay buffer ["obs", "next_obs", "acts", "rewards", "terminals",  "task_idxs"]
...
class SerialMTSAC(SerialTwinSACQ):
    """"
    Support Different Temperature for different tasks
    """
    def __init__(self, task_nums,
