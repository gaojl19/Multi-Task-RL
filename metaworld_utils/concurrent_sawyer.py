...
class ConcurrentSawyerEnv(SawyerXYZEnv):
    '''
        A newly defined concurrent sawyer environment that supports reach-v1 and push-v1
        Consists of sawyer_arm, 0, 0, puck_pos, ini_puck_pos, goal_pos1, goal_pos2
        obs_dim: <sawyer_arm, object1, object2, object3, object4, goal_pos1, goal_pos2>
                 <sawyer_arm, object1, ini_obj1_pos, object3, object4, goal_pos1, goal_pos2>
                 <sawyer_arm, object1, ini_obj1_pos, object2, ini_obj2_pos, goal_pos1, goal_pos2>
        acs_dim: <x, y, z, gripper>
    ...
    def __init__(
        ...
        self.init_config1 = {
            ...
            # 'hand_init_pos': np.array([0.03662452, 0.67840914, 0.05201501]),
            'hand_init_pos': np.array([1, 1, 1]),
        }
        ...
        self.reach_goal = np.array([-0.1, 0.8, 0.2])
        self.push_goal = np.array([0.1, 0.8, 0.02])
        self.pick_place_goal = np.array([0.1, 0.8, 0.2])
        
        print(self.task_type)
        if self.task_type == ['reach', 'push']:
            # self.goal_1 = np.array([-0.1, 0.8, 0.2])
            # self.goal_2 = np.array([0.1, 0.8, 0.02])
            self.goal = np.array([-0.1, 0.8, 0.2, 0.1, 0.8, 0.02])
            
        elif self.task_type == ['push', 'pick_place']:
            # self.goal_1 = np.array([0.1, 0.8, 0.02])
            # self.goal_2 = np.array([0.1, 0.8, 0.2])
            self.goal = np.array([0.1, 0.8, 0.02, 0.1, 0.8, 0.2])
        ...
        self.hand_init_pos = self.init_config1['hand_init_pos']
    ...
    @property
    def model_name(self):
        return get_asset_full_path('sawyer_xyz/concurrent_sawyer_reach_push_pick_and_place.xml')
    ...
    def step(self, action):
        ...
        elif self.rotMode == 'fixed': # here metaworld
        ...
        # print("push dist: ", pushDist)
        # print("place dist: ", placingDist)
        success = {}
        if 'reach' in self.task_type:
            success["reach"] = float(reachDist <= 0.05)
            # success = float(reachDist <= 0.05)
        if 'push' in self.task_type:
            success['push'] = float(pushDist <= 0.07)
        if 'pick_place' in self.task_type:
            success['pick_place'] = float(placingDist <= 0.07)
        info = {'reachDist': reachDist, 'pickRew':pickRew, 'epRew' : reward, 'success': success}
        info['goal'] = self.goal
    ...
    def _set_goal_marker(self, goal):
        ...
        for i in range(len(self.task_type)):
            self.data.site_xpos[self.model.site_name2id('goal_{}'.format(self.task_type[i]))] = (
                goal[(i)*3:(i+1)*3]
            )
    ...
    def _set_objCOM_marker(self):
        ...
        if "push" in self.task_type:
            objPos2 = self.data.get_geom_xpos('objGeom1')
            self.data.site_xpos[self.model.site_name2id('objSite1')] = (
                objPos2
            )
        
        if "pick_place" in self.task_type:
            objPos1 = self.data.get_geom_xpos('objGeom2')
    ...
    def adjust_initObjPos(self, orig_init_pos, body_name, geom_name):
        #This is to account for meshes for the geom and object are not aligned
        #If this is not done, the object could be initialized in an extreme position
        diff = self.get_body_com(body_name)[:2] - self.data.get_geom_xpos(geom_name)[:2]
        
        adjustedPos = orig_init_pos[:2] + diff

        #The convention we follow is that body_com[2] is always 0, and geom_pos[2] is the object height
        return [adjustedPos[0], adjustedPos[1],self.data.get_geom_xpos(geom_name)[-1]]

    # This is where self.reset() will end up
    def reset_model(self):
        ...
        init_pos1 = self.init_config1['obj_init_pos']
        init_pos2 = self.init_config2['obj_init_pos']
        self.maxReachDist = np.linalg.norm(self.init_fingerCOM - self.reach_goal)
        self.maxPickDist = np.linalg.norm(self.init_fingerCOM - np.array(init_pos2))
        self.maxPushDist = np.linalg.norm(init_pos1[:2] - self.push_goal[:2])
        self.maxPlacingDist = np.linalg.norm(np.array([init_pos2[0], init_pos2[1], self.heightTarget2]) - self.pick_place_goal) + self.heightTarget2
        self.target_rewards = [1000*self.maxPlacingDist + 1000*2, 1000*self.maxReachDist + 1000*2, 1000*self.maxPushDist + 1000*2]
        
        idx = []
        if 'reach' in self.task_type:
            idx.append(1)
        elif 'push' in self.task_type:
            idx.append(2)
        else:
            idx.append(0)
            
        for i in idx:
            self.target_reward = self.target_rewards[i]
    ...
    def compute_reward(self, actions, obs, mode = 'general', task_type='reach'):
        ...
        heightTarget = self.heightTarget2
        goal = self._state_goal

        def compute_reward_reach(actions, obs, mode, goal):
        ...
        def compute_reward_push(actions, obs, mode, objPos, goal):
            c1 = 1000 ; c2 = 0.01 ; c3 = 0.001
        ...
        def compute_reward_pick_place(actions, obs, mode, objPos, objHeight, goal):
            ...
            def objDropped():
                return (objPos[2] < (objHeight + 0.005)) and (placingDist >0.02) and (reachDist > 0.02) 
            ...
            def orig_pickReward():       
                ...
                elif (reachDist < 0.1) and (objPos[2]> (objHeight + 0.005)) :
            ...
            def general_pickReward():
                ...
                if self.pickCompleted and objGrasped():
                    return hScale*heightTarget
                elif objGrasped() and (objPos[2]> (objHeight + 0.005)):
                    return hScale* min(heightTarget, objPos[2])
                else:
                    return 0
        ...
        def compute_reward_pick(actions, obs, mode, objPos, goal):
            reachDist = np.linalg.norm(objPos - fingerCOM)
            ...
            return [reward, reachRew, reachDist, None, None, pickRew, None, None]

        # reward , reachRew, reachDist, pushRew, pushDist, pickRew, placeRew , placingDist
        reward = [0, None, None, None, None, None, None, None]
        if 'reach' in task_type:
            rew = compute_reward_reach(actions, obs, mode, goal[:3])
            reward[0] += rew[0]
            reward[1] = rew[1]
            reward[2] = rew[2]
        
        if 'push' in task_type:
            rew = compute_reward_push(actions, obs, mode, objPos2, self.push_goal)
            reward[0] += rew[0]
            reward[3] = rew[3]
            reward[4] = rew[4]
            
        if 'pick_place' in self.task_type:
            rew = compute_reward_pick_place(actions, obs, mode, objPos1, self.objHeight2, self.pick_place_goal)
            reward[0] += rew[0]
            reward[5] = rew[5]
            reward[6] = rew[6]
            reward[7] = rew[7]
