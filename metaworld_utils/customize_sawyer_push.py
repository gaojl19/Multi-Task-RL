...
class CustomizeSawyerEnv(SawyerXYZEnv):
    def __init__(
            self,
            random_init=False,
        ...
        self.init_config = {
            'obj_init_angle': .3,
            'obj_init_pos': np.array([0, 0.3, 0.02]),
    ...
    @property
    def model_name(self):
        return get_asset_full_path('sawyer_xyz/customize_sawyer_push.xml')
    ...
    def step(self, action):
        ...
        reward , reachRew, reachDist, pushRew, pushDist, pickRew, placeRew , placingDist, returnDist = self.compute_reward(action, obs_dict, mode=self.rewMode, task_type=self.task_type)
        ...
        if self.task_type == 'reach':
            ...
            success = float(goal_dist <= 0.07 and returnDist <= 0.05)
        else:
            success = float(goal_dist <= 0.07)
        info = {'reachDist': reachDist, 'pickRew':pickRew, 'epRew' : reward, 'goalDist': goal_dist, 'returnDist': returnDist, 'success': success}
        info['goal'] = self.goal
    ...
    def compute_reward(self, actions, obs, mode = 'general', task_type='reach'):
        ...
        def compute_reward_push(actions, obs, mode):
            ...
            return [reward, reachRew, reachDist, pushRew, pushDist, None, None, None, returnDist]
